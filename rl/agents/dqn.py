...
class DQNAgent(AbstractDQNAgent):
    ...
    def backward(self, observation_0, action, reward, observation_1, terminal):
        ...
        if self.step > self.nb_steps_warmup and self.step % self.train_interval == 0:
            ...
            metrics = self.fit_net()

        return metrics

    def fit_net(self, batches):
        # Compute Q values for mini-batch update.
        if self.enable_double_dqn:
            # According to the paper "Deep Reinforcement Learning with Double Q-learning"
            # (van Hasselt et al., 2015), in Double DQN, the online network predicts the actions
            # while the target network is used to estimate the Q value.
            q_values = self.model.predict_on_batch(batches.state_1)
            ...
            target_q_values = self.target_model.predict_on_batch(batches.state_1)
        ...
        ins = [batches.state_0] if type(self.model.input) is not list else batches.state_0
